...
class CLIP(nn.Module):
    ...
    def forward_ctta3(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        ...
        if len(outputs) % 2 == 1:
            loss_CON = 0.
            confidence_gap = torch.tensor(0.0)
        else:
            ...
            gamma = std_eta / torch.sqrt(torch.tensor(float(modality_query.shape[0])).to(outputs.device))
            #print()
            #print((torch.abs(confidence_gap_all - margin_conf)- 1.96*gamma).shape)
            #loss_CON = args.lambda_con *  (torch.clamp(torch.abs(confidence_gap_all - margin_conf), min=0.0)).mean()# [entropys<=entropy_threshold]
            loss_CON = args.lambda_con *   torch.clamp(torch.norm(confidence_gap - margin_conf, p=2) - args.gamma*gamma, min=0.0)# [entropys<=entropy_threshold]
    ...
    def forward_ctta4(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        ...
        margin, entropy_queue,margin_conf, margin_conf_all =get_current_value_ctta(queue_list, args.real_interval)
        outputs = (modality_query_feat @ modality_gallery_feat.t())
        sim_inter = outputs /args.temperature
        # loss_REM, con_weight=entropy_loss_against_noisy(sim_inter, entropy_queue, if_return_weight=True)
        target_modality_gap=compute_modality_gap(modality_query_feat, modality_gallery_feat)
        ...
        return loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, sim_matrix, confidence_gap, margin_conf
    def forward_ctta_ablation(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval=='i2t':
            modality_gallery_feat_all=self.text_features
            modality_query_feat=self.encode_image(modality_query)
            modality_query_feat=all_gather_with_grad(modality_query_feat)
        ...
        if len(outputs)%2 ==1:
            ...
            confidence_margin = (out_real - out_fake).mean(0)

        return loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, sim_matrix#, target_modality_gap, confidence_margin
    ...
    def forward_tta_untrain(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args, scale):
        ...
        # print(softmax_entropy(sim_matrix/args.temperature).sum(1).mean())
