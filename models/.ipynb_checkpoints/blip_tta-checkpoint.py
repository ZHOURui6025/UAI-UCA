...
class BLIP_Retrieval(nn.Module):
    ...
    def forward_ctta(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval=='i2t':
            modality_gallery_feat_all=self.text_features
            modality_query_feat=self.encode_image(modality_query)
            modality_query_feat=all_gather_with_grad(modality_query_feat)
        ...
        rank_sim, rank_indices = torch.sort(F.softmax(sim_matrix/args.temperature, dim=0), descending=True)
        # print('rank_sim', rank_sim)
        ...
        # print(out_real)
        out_fake = rank_sim[:, args.real_interval:].mean(dim=1)
        # print(out_fake)
        # print(1 - out_real+out_fake)
        loss_con = (1 - out_fake + out_real).mul(con_weight)
        # loss_con = (1 - out_real+out_fake ).mul(con_weight)
        loss_con = args.lambda_con * loss_con.mean()
        return loss_REM, loss_UNI, loss_EMG,loss_con, queue_list, sim_matrix
    def forward_ctta2(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval=='i2t':
            modality_gallery_feat_all=self.text_features
            modality_query_feat=self.encode_image(modality_query)
            modality_query_feat=all_gather_with_grad(modality_query_feat)
        ...
        weight = torch.clamp(torch.tensor(1.0) - entropys.clone().detach() / (entropy_threshold + 1e-3), min=0)  # 0.1,2
        entropys = entropys.mul(weight)  # [entropys<=entropy_threshold]
        rank_sim, rank_indices = torch.sort(outputs, descending=True)
        # print(rank_sim[:, 0], rank_sim[:, 1])
        # rank_sim, rank_indices = torch.sort(F.softmax(sim_inter, dim=0), descending=True)
        # print('rank_sim', rank_sim)
        if args.real_interval == 1:
            # print(1)
            out_real = rank_sim[:, 0]
        ...
        # print(out_real[0],out_fake[0], margin_conf, args.lambda_con*(out_real[0]-out_fake[0]-margin_conf))

        loss_REM = entropys.mul((1 - args.lambda_con*(out_real-out_fake-margin_conf)))#[entropys<=entropy_threshold]
        # loss_con = (1 - out_real+out_fake ).mul(con_weight)
        # loss_con = args.lambda_con * loss_con.mean()
        return loss_REM.mean(0), loss_UNI, loss_EMG, queue_list, sim_matrix

    def forward_ctta3(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval == 'i2t':
            modality_gallery_feat_all = self.text_features
            modality_query_feat = self.encode_image(modality_query)
            modality_query_feat = all_gather_with_grad(modality_query_feat)
    ...
    def forward_ctta5(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval == 'i2t':
            modality_gallery_feat_all = self.text_features
            modality_query_feat = self.encode_image(modality_query)
            modality_query_feat = all_gather_with_grad(modality_query_feat)
        ...
        margin, entropy_queue, margin_conf = get_current_value_ctta5(queue_list, args.real_interval)
        outputs = (modality_query_feat @ modality_gallery_feat.t())
        sim_inter = outputs / args.temperature
        # loss_REM, con_weight=entropy_loss_against_noisy(sim_inter, entropy_queue, if_return_weight=True)
        target_modality_gap = compute_modality_gap(modality_query_feat, modality_gallery_feat)
        ...
        if len(outputs) % 2 == 1:
            ...
            rank_sim, rank_indices = torch.sort(outputs.softmax(1), descending=True)
            print(rank_sim[:10])
            rank_sim, rank_indices = torch.sort(outputs, descending=True)
            print(rank_sim[:10])
            # print(rank_indices.shape)
            ...
            confidence_gap_all = out_real - out_fake
            confidence_gap = confidence_gap_all.mean(0)
            loss_CON = (args.lambda_con * (- (confidence_gap_all - margin_conf.mean(0)))).mean(0)  # [entropys<=entropy_threshold]
            # loss_CON = (args.lambda_con *(confidence_gap_all - margin_conf)**2).mean(
            #     0)  # [entropys<=entropy_threshold]

        return loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, sim_matrix,target_modality_gap,confidence_gap_all, distances, margin_conf
    @torch.no_grad()
    def forward_ctta_untrain(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval == 'i2t':
            modality_gallery_feat_all = self.text_features
            modality_query_feat = self.encode_image(modality_query)
            modality_query_feat = all_gather_with_grad(modality_query_feat)
        ...
        if len(outputs) % 2 == 1:
            ...
            loss_CON = (args.lambda_con * (- (confidence_gap_all - margin_conf.mean(0)))).mean(0)  # [entropys<=entropy_threshold]
        return loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, sim_matrix,target_modality_gap,confidence_gap_all, distances, margin_conf
    def forward_ctta4(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval=='i2t':
            modality_gallery_feat_all=self.text_features
            modality_query_feat=self.encode_image(modality_query)
            modality_query_feat=all_gather_with_grad(modality_query_feat)
        ...
        margin, entropy_queue,margin_conf, margin_conf_all =get_current_value_ctta_conf(queue_list, args.real_interval)
        outputs = (modality_query_feat @ modality_gallery_feat.t())
        sim_inter = outputs /args.temperature
        # loss_REM, con_weight=entropy_loss_against_noisy(sim_inter, entropy_queue, if_return_weight=True)
        target_modality_gap=compute_modality_gap(modality_query_feat, modality_gallery_feat)
        ...
        if len(outputs)%2 ==1:
            ...
            confidence_gap_all = out_real-out_fake
            confidence_gap = confidence_gap_all.mean(0)
            loss_CON=(args.lambda_con*(- (confidence_gap_all-margin_conf))).mean(0)#[entropys<=entropy_threshold]

        return loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, sim_matrix, target_modality_gap, confidence_gap
    def unforward_ctta_visual(self, modality_query, device, queue_list, max_queue_size, update_signal, step, args):
        if args.retrieval=='i2t':
            modality_gallery_feat_all=self.text_features
            modality_query_feat=self.encode_image(modality_query)
            modality_query_feat=all_gather_with_grad(modality_query_feat)
        ...
        # print()
        loss_con = (1 - out_fake + out_real).mul(con_weight)
        loss_con = args.lambda_con * loss_con.mean(0)
        return loss_REM, loss_UNI, loss_EMG,loss_con, queue_list, sim_matrix
