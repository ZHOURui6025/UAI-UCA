...
def forward_and_adapt(modality_query, device, args, metric_logger, queue_list, max_queue_size, update_signal, step, model, optimizer):
    ...
    """
    # print('ablation')
    loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, outputs,target_modality_gap,confidence_gap,distances=model.module.forward_ctta3(modality_query, device, queue_list, max_queue_size, update_signal, step, args)
    # loss = 0.0 #args.lambda_rem * loss_REM   #
    #loss=args.lambda_rem*loss_REM
    ...
    return queue_list, outputs
@torch.enable_grad()  # ensure grads in possible no grad context for testing
def forward_and_adapt5(modality_query, device, args, metric_logger, queue_list, max_queue_size, update_signal, step, model, optimizer):
    """Forward and adapt model on batch of data.

    Measure entropy of the model prediction, take gradients, and update params.
    """
    # print('ablation')
    loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, outputs, target_modality_gap, confidence_gap_all, distances, margin_conf=model.module.forward_ctta5(modality_query, device, queue_list, max_queue_size, update_signal, step, args)
    # loss = 0.0 #args.lambda_rem * loss_REM   #
    loss=args.lambda_rem*loss_REM  + loss_CON + args.lambda_emg*loss_EMG+ args.lambda_uni*loss_UNI##
    #loss=args.lambda_rem*loss_REM  + loss_CON + args.lambda_emg*loss_EMG+ args.lambda_uni*loss_UNI##
    # loss =  loss_REM  +  loss_UNI + loss_EMG+  loss_CON
    ...
    metric_logger.update(loss_EMG=loss_EMG.item())
    metric_logger.update(target_modality_gap=target_modality_gap.item())
    #metric_logger.update(confidence_gap_all=confidence_gap_all)
    metric_logger.update(distances=distances.item())
    #metric_logger.update(margin_conf=margin_conf)
    metric_logger.update(loss_total=loss.item())
    metric_logger.update(lr=optimizer.param_groups[0]["lr"])
    return queue_list, outputs, confidence_gap_all, margin_conf
def forward_and_adapt_ablation(modality_query, device, args, metric_logger, queue_list, max_queue_size, update_signal, step, model, optimizer):
    """Forward and adapt model on batch of data.

    Measure entropy of the model prediction, take gradients, and update params.
    """
    loss_REM, loss_UNI, loss_EMG, loss_CON, queue_list, outputs, target_modality_gap, confidence_margin=model.module.forward_ctta4(modality_query, device, queue_list, max_queue_size, update_signal, step, args)#, target_modality_gap, confidence_margin
    print(loss_REM)
    print(loss_CON)
    print(loss_UNI)
    print(loss_EMG)
    loss=loss_REM  +  loss_CON+  loss_UNI + loss_EMG#
    # loss =  loss_REM  +  loss_UNI + loss_EMG+  loss_CON
    ...
    metric_logger.update(loss_total=loss.item())
    metric_logger.update(target_modality_gap=target_modality_gap.item())
    metric_logger.update(confidence_margin=confidence_margin.item())
    metric_logger.update(lr=optimizer.param_groups[0]["lr"])
    return queue_list, outputs
