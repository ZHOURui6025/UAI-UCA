...
# 原始YAML文件路径
original_yaml_path = 'retrieval_coco.yaml'
...
for annotation_type, sequences in replacements.items():
    if isinstance(sequences, list):  # 如果sequences是一个列表
        for sequence in sequences:
            # 构建新的ann_root路径
            new_ann_root = f'text_perturbation/mscoco/{annotation_type}/{sequence}/'

            # 创建一个新的字典副本，以确保每次替换都是独立的
            new_yaml_content = original_yaml_content.copy()
            new_yaml_content['ann_root'] = new_yaml_content['ann_root'].replace('original_annotation/coco_annotations/', new_ann_root)
